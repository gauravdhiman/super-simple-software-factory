...
11. **Isolate mutating runs; never push; delete only when asked.** A workflow whose agents may write runs in its own worktree on its own branch (`worktree.ensure` in an `isolate` code phase, cut from `--source-branch` / `isolation.source_branch` / `main`); a workflow that commits rebases onto the latest source first (`rebase_onto_source`, guarded by `run.isolation is not None`). Read-only and diff-based workflows stay in place. No ADW pushes, force-updates, or opens a PR — the branch is left rebased and ready for the engineer. Checkout removal is strictly opt-in (`--cleanup` / `isolation.cleanup_on_success`, green runs with clean trees only, via `worktree.maybe_cleanup` after `run.finish()`); branches are never deleted by any ADW.
12. **Every finished run leaves a manifest.** `run.finish()` writes `manifest.json` beside the session and mirrors it to `sessions.manifest_json`: phases, commits, branch, seals. Operators and the UI read the manifest; nobody hand-rolls run summaries, and writing it never fails the run.

## v1 scope

Workers run on a configured harness: `pi`, `muse`, and `opencode` implemented (`coding_agent: pi | muse | opencode`), default model `gemini-3.6-flash` via openrouter, thinking `medium`. `claude_code`, `codex`, `omp` are schema-valid with adapters to come. The visualizer app ships in a later pass — observe via sqlite queries until then.
