...
11. **Isolate mutating runs; never push.** A workflow whose agents may write runs in its own worktree on its own branch (`worktree.ensure` in an `isolate` code phase, cut from `--source-branch` / `isolation.source_branch` / `main`); a workflow that commits rebases onto the latest source first (`rebase_onto_source`, guarded by `run.isolation is not None`). Read-only and diff-based workflows stay in place. No ADW pushes, force-updates, or opens a PR — the branch is left rebased and ready for the engineer.
