...
Workers run on a configured harness: `pi`, `muse`, `opencode`, and `codex` implemented (`coding_agent: pi | muse | opencode | codex`), default model `gemini-3.6-flash` via openrouter, thinking `medium`. `claude_code` and `omp` are schema-valid with adapters to come. The visualizer app ships in a later pass — observe via sqlite queries until then.
