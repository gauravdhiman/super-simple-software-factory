...
| `agent_codex.py` | the Codex interface — `codex exec --json` tailed live, factory session id mapped to a persisted thread id (`exec resume` continues, new events only); system text composed ahead of user text (no system flag); thinking maps to codex vocabulary (`off` → `none`, `minimal` → `low`, unknown → `medium`, never passthrough — codex 400-fails unknown values); `-s workspace-write` + `approval_policy=never`, sandbox stays on; usage parsed from `turn.completed`, cost reads zero |
| `agent_omp.py` | the OMP interface — `omp -p --mode json` tailed live, same pi-family shapes (assistant `message_end` turns, `toolCall` blocks, `tool_execution_*`, pi-shaped `usage`); factory session id mapped to the server-minted id off the opening `session` event (`-r` resumes, full flag set on every send); `--system-prompt`, `--thinking`, `--tools`, `-e`, `--session-dir`, `--cwd` all direct; model resolves against `omp models --json` with pi's ambiguity errors; usage/cost fold turn by turn |
| `agent_cc.py` | the Claude Code interface — `claude -p --verbose --output-format stream-json` tailed live, factory session id mapped to the server-minted id off `system/init` (`--resume` continues); prompt travels as argv (piped stdin yields empty synthetic turns), `--verbose` mandatory for stream-json; tool names translated (`bash` -> `Bash`, `find` -> `Glob`, unmapped dropped); thinking maps to `--effort` (`off`/`minimal` floor to `low`, unknown -> `medium`); `--permission-mode bypassPermissions`, permissions.py stays the boundary; usage/cost real from the terminal `result` event |
