...
| Field | Type | Meaning |
|---|---|---|
| `coding_agent` | `pi` \| `claude_code` \| `opencode` \| `codex` \| `muse` \| `omp` | Which interface runs the agent. All six are implemented and resolve through `harness.py`. Auth is never the factory's business — each CLI uses whatever the operator configured (subscription or keys). |
...
| `tools` | list[string] | Roster-wide tool allowlist. Every agent that omits its own `tools` inherits this. Unset = all tools usable. Pi-only: Muse, OpenCode, and Codex have no allowlist flag, so `tools` is accepted and ignored there — OpenCode runs as its `build` agent with `--auto`, Codex with `-s workspace-write` plus `approval_policy=never`, and the repo boundary stays enforced post-hoc by `permissions.py` on every harness. |
...
Mapped to Pi's reasoning effort control and honored when the model is registered with `reasoning: true` in `~/.pi/agent/models.json`. On a non-reasoning model the setting is inert — no error, no effect. Rough guidance: `high`/`xhigh` for planners and reviewers, `medium` for builders, `low` for mechanical read-and-report agents. (For Claude Code in v2, the same field maps to the thinking budget.) On Muse the ladder maps with `off` → `none`. On OpenCode it passes straight through `--variant` — variants are provider-specific and unknown values are ignored, so every rung is sent verbatim except `off`, which drops the flag and keeps the provider default. On Codex it maps onto codex's effort vocabulary (`off` → `none`, `minimal` → `low`, unrecognized → `medium`) via `-c model_reasoning_effort=` — codex fails unknown values with a 400 instead of ignoring them, so nothing is ever passed through verbatim.

## Model resolution

**Always write `model` as `provider/model-id`.** On Pi, `agents.py` hands the string to the Pi interface, which resolves it against pi's merged catalog — `~/.pi/agent/models.json` plus pi's built-in providers. On Muse, OpenCode, and Codex the id passes through opaquely (non-empty is the only check) and the CLI itself reports an unknown id at spawn time — `opencode models` lists the available `provider/model` ids. The same model is usually carried by more than one provider (`gemini-3.6-flash` lives under `google` *and* under `openrouter` as `google/gemini-3.6-flash`), and a bare id that matches several **raises at resolution**:
...
`harness_engineering` entries are pi extension **file paths**, passed through as `pi -e <path>`, one flag per entry, scoped to that agent only. This is where per-agent harness changes live — e.g. an output-tightening extension for an agent that keeps wrapping its envelope in prose. The starter roster ships with none. On Muse, OpenCode, Codex, and Claude Code the field is reserved (MCP config and hooks when those adapters need it) and currently ignored.
