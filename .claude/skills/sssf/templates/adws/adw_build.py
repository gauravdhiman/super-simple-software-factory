...
Usage:
    uv run adws/adw_build.py "<prompt or path/to/prompt.md>" [--config adws/adw_sssf_config/sssf.config.yaml] [--adw-id a1b2c3d4] [--source-branch main] [--no-worktree] [--cleanup]

Phases: engineer(request) -> git(isolate) -> builder

The code is written in the run's own worktree on branch sssf/<adw_id> and left
uncommitted there — this workflow never commits, rebases, or pushes, so the
change stays where the engineer can review it.
"""

import argparse
import sys

from adw_modules import agents, gates, session, utils, worktree
